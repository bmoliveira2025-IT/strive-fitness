        ...
        <View style={{ flex: 1, backgroundColor: 'transparent' }}>
            ...
            >
                <View className="flex-row items-center justify-between px-6 mb-2">
                    <View className="flex-row items-center">
                        ...
                        <View className="ml-4">
                            <Text style={{ color: theme.colors.text }} className="text-2xl font-bold italic uppercase tracking-tighter">Músculos</Text>
            ...
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 120 }}>
                ...
                <View className="px-6 mb-10 pt-4">
                    <Text style={{ color: theme.colors.text }} className="text-[10px] font-bold uppercase tracking-[3px] mb-4 opacity-50 pl-1">FOCO RECOMENDADO</Text>
                    {suggestions.length > 0 ? (
                        <View className="gap-4">
                            {suggestions.map((muscle, idx) => (
                                ...
                                >
                                    ...
                                    >
                                        <LinearGradient
                                            colors={[theme.colors.warning + '15', 'transparent']}
                                            start={{ x: 0, y: 0 }}
                                            end={{ x: 1, y: 1 }}
                                        ...
                                        <View className="flex-row justify-between items-center mb-4">
                                            <View className="flex-row items-center">
                                                <View style={{ backgroundColor: theme.colors.warning + '20', width: 40, height: 40, borderRadius: 14, alignItems: 'center', justifyContent: 'center', marginRight: 15 }}>
                                                    <Ionicons name="flash" size={18} color={theme.colors.warning} />
                                                </View>
                                                <View>
                                                    <Text style={{ color: theme.colors.text }} className="font-bold italic text-lg uppercase leading-5">{muscle.name}</Text>
                                                    <View className="flex-row items-center mt-0.5">
                                                        <View className="w-1.5 h-1.5 rounded-full bg-yellow-500 mr-2" />
                                                        <Text className="text-yellow-600 text-[9px] font-bold uppercase tracking-widest">Subtreinado</Text>
                                                    </View>
                                                </View>
                                            </View>
                        ...
                        <View style={{ backgroundColor: theme.colors.card, borderColor: theme.colors.cardBorder, borderWidth: 1.5 }} className="p-8 rounded-[32px] items-center">
                            ...
                            <Text style={{ color: theme.colors.text }} className="font-bold italic uppercase tracking-tight text-center mt-3">Estado Ótimo</Text>
                ...
                <View className="px-6">
                    <Text style={{ color: theme.colors.text }} className="text-[10px] font-bold uppercase tracking-[3px] mb-4 opacity-50 pl-1">STATUS GERAL</Text>
                        ...
                        >
                            ...
                            >
                                <View className="flex-row items-center">
                                    ...
                                    <View>
                                        <Text style={{ color: theme.colors.text }} className="font-bold italic uppercase tracking-tight text-base mb-1">{muscle.name}</Text>
                                        <View className="flex-row items-center">
                                            <Ionicons name="layers-outline" size={10} color={theme.colors.textMuted} />
                                            <Text style={{ color: theme.colors.textMuted }} className="text-[9px] font-bold uppercase tracking-widest ml-1">{muscle.setsThisWeek} séries por semana</Text>
                                        </View>
                                    </View>
                                </View>
                                <View className="items-end bg-black/5 dark:bg-white/5 px-4 py-2 rounded-2xl border border-black/5 dark:border-white/5">
                                    <Text style={{ color: theme.colors.text }} className="font-bold italic text-xl tracking-tighter leading-6">{muscle.score}</Text>
                        ...
                        {selectedMuscle && (
                            <>
                                <View className="items-center mb-8">
                                    ...
                                    <Text style={{ color: theme.colors.text }} className="text-3xl font-bold italic uppercase tracking-tighter mb-2">{selectedMuscle.name}</Text>
                                    <View
                                        style={{
                                            backgroundColor: selectedMuscle.status === 'recovered' ? theme.colors.success + '15' :
                                                selectedMuscle.status === 'undertrained' ? theme.colors.warning + '15' :
                                                    selectedMuscle.status === 'overreaching' ? theme.colors.error + '15' : theme.colors.info + '15',
                                            borderColor: selectedMuscle.status === 'recovered' ? theme.colors.success + '30' :
                                                selectedMuscle.status === 'undertrained' ? theme.colors.warning + '30' :
                                                    selectedMuscle.status === 'overreaching' ? theme.colors.error + '30' : theme.colors.info + '30',
                                            borderWidth: 1.5
                                        }}
                                    ...
                                    >
                                        <Text
                                            style={{
                                                color: selectedMuscle.status === 'recovered' ? theme.colors.success :
                                                    selectedMuscle.status === 'undertrained' ? theme.colors.warning :
                                                        selectedMuscle.status === 'overreaching' ? theme.colors.error : theme.colors.info
                                            }}
                                ...
                                >
                                    <View className="flex-row items-center mb-2">
                                        ...
                                        <Text style={{ color: theme.colors.primary }} className="text-[10px] font-bold uppercase tracking-widest ml-2">Stricia AI Insights</Text>
                                    </View>
                                    ...
                                    {[
                                        ...
                                        >
                                            <View className="flex-row items-center mb-2 opacity-50">
                                                ...
                                                <Text style={{ color: theme.colors.textMuted }} className="text-[8px] font-bold uppercase ml-1.5">{metric.label}</Text>
                                            </View>
                                            <View className="flex-row items-baseline">
                                                <Text style={{ color: theme.colors.text }} className="text-xl font-bold italic tracking-tighter">{metric.val}</Text>
                                ...
                                >
                                    <Text className="text-onPrimary font-bold uppercase tracking-widest text-[12px]">Confirmar Feedback</Text>
                                </GradientButton>
                            </>
                        )}
                    </Animated.View>
